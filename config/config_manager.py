...
class ConfigManager:
    ...
    def _validate_and_convert(self, data: Dict[str, Any]) -> Config:
        ...
        logging_config = LoggingConfig(
            ...
            rotation=logging_data.get("rotation", {"when": "W0", "interval": 1, "backup_count": 4}),
            format=logging_data.get("format", "%(asctime)s | %(levelname)s | %(message)s"),
        )

        return Config(kubernetes=kubernetes_config, helm=helm_config, logging=logging_config)
