...
from .exceptions import (
    CleanupError, HelmError, HelmUninstallError,
...
def list_helm_releases(namespace: str) -> List[Dict[str, Any]]:
    ...
    try:
        result = run_command(
            ["helm", "list", "--namespace", namespace, "--output", "json", "--time-format", "2006-01-02T15:04:05Z07:00"],
            capture=True
        )
        ...
        logger.info("Found Helm releases", extra={
            "namespace": namespace,
            "count": len(releases)
        })
        return releases
...
def filter_old_pr_releases(releases: List[Dict[str, Any]], prefix: str, days_threshold: int) -> List[str]:
    ...
    logger.info("Filtering old PR releases", extra={
        "prefix": prefix,
    ...
    for release in releases:
        ...
        if not release_name or not updated_str:
            logger.warning("Skipping release with missing data", extra={
                "release": release
            })
            continue
        ...
        if release_name.startswith(prefix):
            try:
                ...
                if updated_time.tzinfo is None:
                     logger.warning("Release updated time lacks timezone", extra={
                ...
                if updated_time < cutoff_time:
                    logger.debug("Found old PR release", extra={
                        "release": release_name,
                        "updated_time": updated_time.isoformat()
                    })
                    old_releases.append(release_name)
            except ValueError:
                logger.warning("Could not parse update timestamp", extra={
                    "release": release_name,
                    "updated_time": updated_str
                })
            except Exception as e:
                logger.warning("Error processing release", extra={
                    "release": release_name,
                    "error": str(e)
                })

    logger.info("Found releases matching criteria", extra={
...
def delete_helm_release(release_name: str, namespace: str) -> None:
    ...
    """
    logger.info("Deleting Helm release", extra={
        "release": release_name,
        "namespace": namespace
    })
    try:
        run_command(["helm", "uninstall", release_name, "--namespace", namespace])
        logger.info("Helm uninstall command ran", extra={
            "release": release_name
        })
